...
San Francisco, CA 94102`,logoUrl:"",logoInitial:"A"},invoice:{number:"INV-2026-0087",date:"January 27, 2026",dueDate:"February 26, 2026",notes:"Thank you for your prompt payment.",paymentTerms:"Payment due within 30 days. Late fees of 1.5% per month apply."},billTo:{name:"Sarah Chen",company:"Brightpath Analytics",email:"sarah@brightpath.io",address:"789 Data Drive, Austin, TX 73301"},lineItems:[{description:"Data Pipeline Setup",details:"ETL pipeline configuration and testing",quantity:1,rate:"4,500.00",amount:"4,500.00"},{description:"Dashboard Development",details:"Custom analytics dashboards (3 views)",quantity:3,rate:"2,000.00",amount:"6,000.00"},{description:"API Integration",details:"Third-party API connectors",quantity:2,rate:"1,500.00",amount:"3,000.00"}],totals:{subtotal:"13,500.00",tax:"1,080.00",total:"14,580.00"},styles:{accentColor:"#2563eb"}},sampleReceiptData={merchant:{name:"The Daily Grind",address:"456 Market St, San Francisco, CA 94102"},receipt:{number:"REC-2026-0193",date:"January 27, 2026",time:"10:32 AM"},items:[{name:"Monthly Subscription - Pro Plan",quantity:1,price:"49.00"},{name:"Additional Storage (50GB)",quantity:1,price:"10.00"}],totals:{subtotal:"59.00",tax:"4.72",total:"63.72"},payment:{method:"Credit Card ending in 4242"},styles:{accentColor:"#059669"}},sampleReportData={report:{title:"Q4 2025 Performance Review",subtitle:"Trends, Opportunities, and Strategic Recommendations",author:"Strategy Team",date:"January 27, 2026",organization:"Acme Corporation"},styles:{accentColor:"#7c3aed"}},TEMPLATE_TYPE_CONFIG={quote:{variants:[{id:"quote-modern",label:"Modern"},{id:"quote-professional",label:"Professional"},{id:"quote-bold",label:"Bold"}],data:sampleQuoteData},invoice:{variants:[{id:"invoice-clean",label:"Clean"}],data:sampleInvoiceData},receipt:{variants:[{id:"receipt-minimal",label:"Minimal"}],data:sampleReceiptData},report:{variants:[{id:"report-cover",label:"Cover"}],data:sampleReportData}};function getCurrentSampleData(){for(const[e,t]of Object.entries(TEMPLATE_TYPE_CONFIG))if(t.variants.some(o=>o.id===currentTemplate))return t.data;return sampleQuoteData}let isInDemoMode=!1;function updateSkeletonStage(e){const t=document.querySelectorAll(".preview-init-loading__stage"),o=["connecting","loading","ready"],n=o.indexOf(e);t.forEach(i=>{const a=i.dataset.stage,r=o.indexOf(a);i.classList.remove("active","completed"),r<n?i.classList.add("completed"):r===n&&i.classList.add("active")})}function hideInitialLoadingSkeleton(){const e=document.getElementById("preview-init-loading");e&&(updateSkeletonStage("ready"),setTimeout(()=>{e.classList.add("hidden"),setTimeout(()=>{e.style.display="none"},400)},300))}function showInitialLoadingSkeleton(){const e=document.getElementById("preview-init-loading");e&&(e.style.display="flex",e.classList.remove("hidden"),updateSkeletonStage("connecting"))}function showDemoModeBanner(){const e=document.getElementById("demo-mode-banner");e&&(e.classList.add("visible"),isInDemoMode=!0)}function hideDemoModeBanner(){const e=document.getElementById("demo-mode-banner");e&&(e.classList.remove("visible"),isInDemoMode=!1)}function setupDemoModeBanner(){const e=document.getElementById("demo-mode-retry"),t=document.getElementById("demo-mode-dismiss");e&&e.addEventListener("click",async()=>{e.textContent="Retrying...",e.disabled=!0,hideDemoModeBanner(),await initializePreview(),e.textContent="Retry Connection",e.disabled=!1}),t&&t.addEventListener("click",()=>{hideDemoModeBanner()})}async function initializePreview(){showInitialLoadingSkeleton(),setStatus("Connecting..."),updateSkeletonStage("connecting");try{setTimeout(()=>updateSkeletonStage("loading"),400);const e=await fetch(`${API_URL}/v1/preview`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${DEMO_API_KEY}`},body:JSON.stringify({template:currentTemplate,data:getCurrentSampleData()})});if(!e.ok)throw new Error("Failed to load preview");const t=await e.json();if(currentHtml=t.html,sessionId=t.sessionId,setStatus("Loading preview..."),hideDemoModeBanner(),renderPreview(currentHtml),hideInitialLoadingSkeleton(),setStatus("Ready"),startSessionTimer(),window.innerWidth<=480){const o=document.getElementById("preview-container");o&&o.classList.add("zoomed-in")}}catch(e){if(console.warn("API not available, using static demo:",e),updateSkeletonStage("loading"),setStatus("Loading demo..."),renderStaticDemo(),hideInitialLoadingSkeleton(),setStatus("Demo Mode"),showDemoModeBanner(),startSessionTimer(),window.innerWidth<=480){const t=document.getElementById("preview-container");t&&t.classList.add("zoomed-in")}}}function showPreviewError(){const e=previewFrame.parentElement;if(!e||e.querySelector(".glyph-preview-error"))return;const t=document.createElement("div");t.className="glyph-preview-error",t.innerHTML=`
      ...
      `,t.style.cssText="position: absolute; top: 0; left: 0; right: 0; bottom: 0; z-index: 10;",previewFrame.style.display="none",e.style.position="relative",e.appendChild(t)}function renderPreview(e,t={}){const{changes:o=[],previousHtml:n=null}=t;let i;try{if(i=previewFrame.contentDocument||previewFrame.contentWindow.document,!i)throw new Error("contentDocument is null")}catch(a){console.error("[Glyph] Preview frame access denied:",a),showPreviewError();return}try{i.open(),i.write(e),i.close()}catch(a){console.error("[Glyph] Preview frame write failed:",a),showPreviewError();return}(o.length>0||n)&&highlightChangedRegions(i,o,n,e)}function highlightChangedRegions(e,t,o,n){const i="glyph-change-highlight-styles";if(!e.getElementById(i)){const l=e.createElement("style");l.id=i,l.textContent=`
          .glyph-changed {
            position: relative;
        ...
        `,e.head.appendChild(l)}const a=e.querySelectorAll("[data-glyph-region]");let r=0;const s=extractChangeKeywords(t);a.forEach(l=>{const p=l.getAttribute("data-glyph-region").toLowerCase();s.some(d=>p.includes(d)||d.includes(p))&&(applyHighlight(l),r++)}),r===0&&o&&(r=highlightByDiff(e,o,n)),r===0&&t.length>0&&highlightByKeywordSearch(e,s),console.log(`[Glyph] Highlighted ${r} changed regions`)}function extractChangeKeywords(e){const t=[],o=[/header/i,/footer/i,/title/i,/subtitle/i,/logo/i,/color/i,/background/i,/font/i,/text/i,/border/i,/button/i,/link/i,/image/i,/icon/i,/table/i,/price/i,/total/i,/item/i,/product/i,/service/i,/company/i,/contact/i,/address/i,/phone/i,/email/i,/date/i,/number/i,/signature/i,/qr/i,/code/i,/payment/i,/terms/i,/notes/i,/description/i];return e.forEach(n=>{const i=n.toLowerCase();o.forEach(r=>{const s=i.match(r);s&&t.push(s[0].toLowerCase())}),(i.match(/\b[a-z]{4,}\b/gi)||[]).forEach(r=>{["added","updated","changed","modified","removed","style","styling"].includes(r.toLowerCase())||t.push(r.toLowerCase())})}),[...new Set(t)]}function applyHighlight(e){e.classList.add("glyph-changed"),setTimeout(()=>{e.classList.remove("glyph-changed")},2500)}function highlightByKeywordSearch(e,t){if(t.length===0)return;const o=["h1","h2","h3","h4","h5","h6","header","footer","section","article",".header",".footer",".title",".subtitle",'[class*="header"]','[class*="title"]','[class*="footer"]','[class*="logo"]','[class*="price"]','[class*="total"]'];e.querySelectorAll(o.join(", ")).forEach(i=>{const a=(i.className||"").toLowerCase(),r=(i.id||"").toLowerCase(),s=(i.textContent||"").toLowerCase().substring(0,100);t.some(p=>a.includes(p)||r.includes(p)||s.includes(p))&&applyHighlight(i)})}function highlightByDiff(e,t,o){let n=0;const i=document.createElement("div"),a=document.createElement("div");return i.innerHTML=t,a.innerHTML=o,e.querySelectorAll("[data-glyph-region]").forEach(l=>{const p=l.getAttribute("data-glyph-region"),c=i.querySelector(`[data-glyph-region="${p}"]`);(!c||c.innerHTML!==l.innerHTML)&&(applyHighlight(l),n++)}),e.querySelectorAll("h1, h2, h3, .header, .title, table, .total").forEach(l=>{const p=i.querySelectorAll(l.tagName);let c=!1;p.forEach(d=>{d.outerHTML===l.outerHTML&&(c=!0)}),!c&&!l.classList.contains("glyph-changed")&&(applyHighlight(l),n++)}),n}function saveToUndoHistory(){currentHtml&&(undoHistory.push(currentHtml),redoHistory=[],undoHistory.length>MAX_UNDO_HISTORY&&undoHistory.shift(),updateUndoButtonState())}function performUndo(){if(undoHistory.length===0)return;currentHtml=undoHistory.pop(),renderPreview(currentHtml),refreshAppliedActionsFromHtml(currentHtml),previewContainer.classList.add("success"),setTimeout(()=>previewContainer.classList.remove("success"),500),flashPreviewArea(),setStatus("Undone"),console.log("[Glyph] Undo performed, history length:",undoHistory.length),setTimeout(()=>setStatus("Ready"),1500),updateUndoButtonState(),clearDiffState()}function updateUndoButtonState(){undoBtn&&(undoBtn.disabled=undoHistory.length===0,undoBtn.title=undoHistory.length>0?`Undo last change (${undoHistory.length} available)`:"No changes to undo"),updateShareAndSaveButtonState()}function updateShareAndSaveButtonState(){const e=undoHistory.length>0,t=document.getElementById("share-btn"),o=document.getElementById("save-version-btn");t&&(t.disabled=!e,t.title="Share this customization",t.dataset.tooltip=e?"":"Make a change to share"),o&&(o.disabled=!e,o.title="Save named version",o.dataset.tooltip=e?"":"Make a change to save a version")}function clearUndoHistory(){undoHistory=[],updateUndoButtonState(),clearAllAppliedActions(),clearDiffState()}function snapshotRegions(e){const t=document.createElement("div");t.innerHTML=e;const o=new Map;return t.querySelectorAll("[data-glyph-region]").forEach(n=>{o.set(n.getAttribute("data-glyph-region"),n.innerHTML)}),o}function captureDiffBefore(){currentHtml&&(diffRegionSnapshots={before:snapshotRegions(currentHtml),after:null},isDiffActive&&toggleDiffView(!1))}function captureDiffAfter(){!diffRegionSnapshots||!currentHtml||(diffRegionSnapshots.after=snapshotRegions(currentHtml),updateDiffButtonState())}function getChangedRegionNames(){if(!diffRegionSnapshots||!diffRegionSnapshots.before||!diffRegionSnapshots.after)return[];const e=[],t=diffRegionSnapshots.after,o=diffRegionSnapshots.before;return t.forEach((n,i)=>{const a=o.get(i);(a===void 0||a!==n)&&e.push(i)}),o.forEach((n,i)=>{t.has(i)||e.push(i)}),e}function toggleDiffView(e){isDiffActive=e!==void 0?e:!isDiffActive,diffToggleBtn&&diffToggleBtn.classList.toggle("active",isDiffActive);const o=previewFrame;if(!o||!o.contentDocument)return;const n=o.contentDocument,i="glyph-diff-toggle-styles";if(!isDiffActive){const r=n.getElementById(i);r&&r.remove(),n.querySelectorAll(".glyph-diff-changed").forEach(s=>{s.classList.remove("glyph-diff-changed")});return}if(!n.getElementById(i)){const r=n.createElement("style");r.id=i,r.textContent=`
          .glyph-diff-changed {
        ...
        `;return}const e=historyEntries.slice().reverse().map((t,o)=>{const n=historyEntries.length-1-o,i=n===currentHistoryIndex;return`
            <div class="history-entry${i?" history-entry--current":""}" data-index="${n}">
...
print(result['html'])  # Updated HTML`,l=document.getElementById("code-snippet-curl"),p=document.getElementById("code-snippet-javascript"),c=document.getElementById("code-snippet-python");l&&(l.textContent=a),p&&(p.textContent=r),c&&(c.textContent=s)}function initCopyCodeModal(){const e=document.getElementById("copy-code-modal");if(!e)return;const t=document.getElementById("copy-code-close");t&&t.addEventListener("click",closeCopyCodeModal),e.addEventListener("click",a=>{a.target===e&&closeCopyCodeModal()}),document.addEventListener("keydown",a=>{a.key==="Escape"&&e.classList.contains("active")&&closeCopyCodeModal()});const o=e.querySelectorAll(".copy-code-modal__tab"),n=e.querySelectorAll(".copy-code-modal__snippet");o.forEach(a=>{a.addEventListener("click",()=>{const r=a.dataset.lang;o.forEach(s=>s.classList.remove("copy-code-modal__tab--active")),a.classList.add("copy-code-modal__tab--active"),n.forEach(s=>{s.dataset.lang===r?s.classList.add("copy-code-modal__snippet--active"):s.classList.remove("copy-code-modal__snippet--active")})})}),e.querySelectorAll(".copy-code-modal__copy-btn").forEach(a=>{a.addEventListener("click",async()=>{const r=a.dataset.lang,s=document.getElementById(`code-snippet-${r}`);if(!s)return;if(await copyTextToClipboard(s.textContent)){const p=a.innerHTML;a.classList.add("copy-code-modal__copy-btn--copied"),a.innerHTML=`
              <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
              </svg>
              Copied!
            `,setTimeout(()=>{a.classList.remove("copy-code-modal__copy-btn--copied"),a.innerHTML=p},2e3)}else showToast("Couldn't copy to clipboard - try Ctrl/Cmd+C","error",4e3)})}),console.log("[Glyph] Copy as Code modal initialized")}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",initCopyCodeModal):initCopyCodeModal();let modificationSuccessToastElement=null,modificationSuccessToastTimeout=null;function updateResponseTimeBadge(e,t=!1){const o=document.getElementById("response-time-badge");if(!o)return;const n=parseFloat(e),i=t||n<.5;o.textContent=t?"Instant":`${e}s`,o.className="playground__response-time"+(i?" playground__response-time--fast":""),o.style.display=""}function showModificationSuccessToast(e,t=!1){const o=isFirstWin();if(!modificationSuccessToastElement){modificationSuccessToastElement=document.createElement("div"),modificationSuccessToastElement.className="modification-success-toast",modificationSuccessToastElement.innerHTML=`
        ...
        `,document.body.appendChild(modificationSuccessToastElement);const r=modificationSuccessToastElement.querySelector("#toast-get-code-btn");r&&r.addEventListener("click",s=>{s.stopPropagation(),modificationSuccessToastElement.classList.remove("visible"),modificationSuccessToastTimeout&&clearTimeout(modificationSuccessToastTimeout),openCopyCodeModal()})}const n=modificationSuccessToastElement.querySelector(".modification-success-toast__title"),i=modificationSuccessToastElement.querySelector(".modification-success-toast__detail");if(o?(n&&(n.textContent="You just customized your first PDF!"),i&&(i.textContent="That was all you. Try another change!"),markFirstWinComplete()):t?(n&&(n.textContent="Changes applied instantly!"),i&&(i.textContent="Local transformation - no API call needed")):(n&&(n.textContent="Changes applied successfully"),i&&(i.textContent=`Completed in ${e}s`)),modificationSuccessToastTimeout&&clearTimeout(modificationSuccessToastTimeout),requestAnimationFrame(()=>{modificationSuccessToastElement.classList.add("visible")}),modificationSuccessToastTimeout=setTimeout(()=>{modificationSuccessToastElement.classList.remove("visible")},o?5e3:4e3),window.innerWidth<=480){const r=document.querySelector(".playground__preview-area");r&&setTimeout(()=>{r.scrollIntoView({behavior:"smooth",block:"start"})},300),o&&previewContainer&&!previewContainer.classList.contains("zoomed-in")&&setTimeout(()=>{previewContainer.classList.add("zoomed-in")},800)}}function formatTimestamp(e){const t=new Date(e),n=new Date-t,i=Math.floor(n/6e4),a=Math.floor(n/36e5),r=Math.floor(n/864e5);return i<1?"Just now":i<60?i+" min ago":a<24?a+" hour"+(a===1?"":"s")+" ago":r<7?r+" day"+(r===1?"":"s")+" ago":t.toLocaleDateString("en-US",{month:"short",day:"numeric"})}function renderSavedVersions(){const e=getSaveVersionElements();if(!e.savedVersionsList||!e.savedVersionsCount)return;if(e.savedVersionsCount.textContent=savedVersions.length+"/10",savedVersions.length===0){e.savedVersionsList.innerHTML='<div class="saved-versions__empty">Save your best versions <span class="saved-versions__slots">'+(MAX_SAVED_VERSIONS-savedVersions.length)+" slots available</span></div>";return}const t=[...savedVersions].sort((o,n)=>n.timestamp-o.timestamp);e.savedVersionsList.innerHTML=t.map(o=>{const n=document.createElement("div");return n.className="saved-version-item",n.setAttribute("data-id",o.id),n.innerHTML='<svg class="saved-version-item__star" fill="currentColor" viewBox="0 0 24 24"><path d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/></svg><div class="saved-version-item__info"><div class="saved-version-item__name">'+escapeHtml(o.name)+'</div><div class="saved-version-item__date">'+formatTimestamp(o.timestamp)+'</div></div><button class="saved-version-item__delete" data-id="'+o.id+'" title="Delete version"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>',n.outerHTML}).join(""),e.savedVersionsList.querySelectorAll(".saved-version-item").forEach(o=>{o.addEventListener("click",n=>{if(n.target.closest(".saved-version-item__delete"))return;const i=o.getAttribute("data-id");restoreSavedVersion(i)})}),e.savedVersionsList.querySelectorAll(".saved-version-item__delete").forEach(o=>{o.addEventListener("click",n=>{n.stopPropagation();const i=o.getAttribute("data-id");confirmDeleteVersion(i)})})}function escapeHtml(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}function openSaveVersionModal(){const e=getSaveVersionElements();if(!currentHtml){showToast("No document to save","error");return}if(savedVersions.length>=MAX_SAVED_VERSIONS){showToast("Maximum 10 versions. Delete one first.","error");return}e.versionNameInput&&(e.versionNameInput.value=""),e.saveVersionModal&&e.saveVersionModal.classList.add("active"),setTimeout(()=>e.versionNameInput&&e.versionNameInput.focus(),100)}function closeSaveVersionModal(){const e=getSaveVersionElements();e.saveVersionModal&&e.saveVersionModal.classList.remove("active"),e.versionNameInput&&(e.versionNameInput.value="")}const FIRST_SAVE_KEY="glyph_first_save_complete";function isFirstSave(){try{return!localStorage.getItem(FIRST_SAVE_KEY)}catch{return!1}}function markFirstSaveComplete(){try{localStorage.setItem(FIRST_SAVE_KEY,"true")}catch{}}function saveCurrentVersion(){const e=getSaveVersionElements(),t=e.versionNameInput?e.versionNameInput.value.trim():"";if(!t){showToast("Please enter a name","error");return}if(!currentHtml){showToast("No document to save","error");return}const o=isFirstSave(),n=MAX_SAVED_VERSIONS-savedVersions.length-1,i={id:Date.now().toString(36)+Math.random().toString(36).substr(2,5),name:t,html:currentHtml,timestamp:Date.now()};savedVersions.push(i),persistSavedVersions(),renderSavedVersions(),closeSaveVersionModal();const a=document.getElementById("save-version-btn");a&&a.classList.remove("ready-to-save"),o?(markFirstSaveComplete(),showToast("First version saved! "+n+" slots remaining","success",4e3),celebrateFirstSave()):showToast('Version saved as "'+t+'"',"success"),console.log("[Glyph] Saved version:",t)}function celebrateFirstSave(){const e=document.getElementById("saved-versions-section");e&&(e.classList.add("first-save-glow"),setTimeout(()=>{e.classList.remove("first-save-glow")},1e3))}function restoreSavedVersion(e){const t=savedVersions.find(o=>o.id===e);t&&(saveToUndoHistory(),currentHtml=t.html,renderPreview(currentHtml),previewContainer.classList.add("success"),setTimeout(()=>previewContainer.classList.remove("success"),500),flashPreviewArea(),setStatus("Restored"),setTimeout(()=>setStatus("Ready"),1500),showToast('Restored "'+t.name+'"',"success"),console.log("[Glyph] Restored version:",t.name))}function confirmDeleteVersion(e){const t=getSaveVersionElements(),o=savedVersions.find(n=>n.id===e);o&&(versionToDelete=e,t.deleteVersionName&&(t.deleteVersionName.textContent=o.name),t.deleteConfirmModal&&t.deleteConfirmModal.classList.add("active"))}function closeDeleteModal(){const e=getSaveVersionElements();e.deleteConfirmModal&&e.deleteConfirmModal.classList.remove("active"),versionToDelete=null}function deleteSavedVersion(){if(!versionToDelete)return;const e=savedVersions.find(o=>o.id===versionToDelete),t=e?e.name:"version";savedVersions=savedVersions.filter(o=>o.id!==versionToDelete),persistSavedVersions(),renderSavedVersions(),closeDeleteModal(),showToast('Deleted "'+t+'"',"success"),console.log("[Glyph] Deleted version:",t)}function initSaveVersionListeners(){const e=getSaveVersionElements();e.saveVersionBtn&&e.saveVersionBtn.addEventListener("click",openSaveVersionModal),e.saveVersionCancel&&e.saveVersionCancel.addEventListener("click",closeSaveVersionModal),e.saveVersionConfirm&&e.saveVersionConfirm.addEventListener("click",saveCurrentVersion),e.saveVersionModal&&e.saveVersionModal.addEventListener("click",t=>{t.target===e.saveVersionModal&&closeSaveVersionModal()}),e.versionNameInput&&e.versionNameInput.addEventListener("keypress",t=>{t.key==="Enter"&&saveCurrentVersion()}),e.deleteCancel&&e.deleteCancel.addEventListener("click",closeDeleteModal),e.deleteConfirm&&e.deleteConfirm.addEventListener("click",deleteSavedVersion),e.deleteConfirmModal&&e.deleteConfirmModal.addEventListener("click",t=>{t.target===e.deleteConfirmModal&&closeDeleteModal()}),loadSavedVersions()}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",initSaveVersionListeners):setTimeout(initSaveVersionListeners,0);function renderStaticDemo(){const e=generateStaticHtml(sampleQuoteData,"#1E3A5F");currentHtml=e,renderPreview(e)}function generateStaticHtml(e,t){return`<!DOCTYPE html>
<html>
<head>
<style>
    ...
    <tbody>
      ...
      <tr>
        <td>
          ...
          <div class="item-details">${o.details}</div>
        </td>
        <td>${o.quantity}</td>
        <td>$${o.unitPrice}</td>
        <td>$${o.total}</td>
...
</html>`}function setStatus(e){previewStatus.innerHTML=`<span class="status-dot"></span>${e}`}function setStepperStep(e){const t=document.querySelectorAll(".stepper__step"),o=document.querySelectorAll(".stepper__connector"),i=["analyze","generate","render"].indexOf(e);i!==-1&&(t.forEach((a,r)=>{a.classList.remove("active","completed"),r<i?a.classList.add("completed"):r===i&&a.classList.add("active")}),o.forEach((a,r)=>{a.classList.toggle("done",r<i)}))}function completeAllSteps(){document.querySelectorAll(".stepper__step").forEach(e=>{e.classList.remove("active"),e.classList.add("completed")}),document.querySelectorAll(".stepper__connector").forEach(e=>{e.classList.add("done")})}function resetStepper(){document.querySelectorAll(".stepper__step").forEach(e=>{e.classList.remove("active","completed")}),document.querySelectorAll(".stepper__connector").forEach(e=>{e.classList.remove("done")})}function updateProgressivePreview(){}function clearProgressivePreview(){}let elapsedTimerInterval=null,loadingStartTime=null,currentAbortController=null,cancelButtonTimeout=null;function resetLoadingStages(){resetStepper()}function activateStage(e){setStepperStep({analyze:"analyze",generate:"generate",apply:"generate",validate:"render"}[e]||e)}function showLoading(e){previewLoading.classList.toggle("visible",e);const t=document.getElementById("loading-cancel-btn");e?(resetStepper(),loadingStartTime=Date.now(),setStepperStep("analyze"),t&&t.classList.remove("visible"),cancelButtonTimeout&&(clearTimeout(cancelButtonTimeout),cancelButtonTimeout=null),cancelButtonTimeout=setTimeout(()=>{t&&isProcessingModification&&t.classList.add("visible")},5e3),elapsedTimerInterval&&(clearInterval(elapsedTimerInterval),elapsedTimerInterval=null),elapsedTimerInterval=setInterval(()=>{const o=Math.floor((Date.now()-loadingStartTime)/1e3);setStatus(`Applying... ${o}s`)},1e3)):(elapsedTimerInterval&&(clearInterval(elapsedTimerInterval),elapsedTimerInterval=null),cancelButtonTimeout&&(clearTimeout(cancelButtonTimeout),cancelButtonTimeout=null),t&&t.classList.remove("visible"),completeAllSteps(),setTimeout(()=>{resetStepper()},500))}function isAiRefusalResponse(e){if(!e||typeof e!="string")return!1;const t=e.trim(),o=t.includes("<!DOCTYPE")||t.includes("<html")||t.includes("<body")||t.includes("<div")||t.includes("<table"),n=[/I understand your request/i,/I cannot|I can't|I'm unable to/i,/I apologize|I'm sorry/i,/Unfortunately,? I/i,/This (request|feature|functionality) (is|isn't|cannot)/i,/beyond my capabilities/i,/not possible to/i,/I'd be happy to help.*but/i,/This would require/i,/PDF documents cannot/i,/interactive (features|elements|charts)/i,/video (player|playback|content)/i,/animations? (are|is) not/i,/real-time (data|updates)/i];if(!o){for(const a of n)if(a.test(e))return!0;if(t.length<500)return!0}const i=e.replace(/<[^>]*>/g,"").trim();if(i.length>100){for(const a of n)if(a.test(i.substring(0,500)))return!0}return!1}function extractRefusalMessage(e){const o=e.replace(/<[^>]*>/g," ").replace(/\s+/g," ").trim().split(".")[0];return o.length>100?o.substring(0,100)+"...":o||"This request cannot be completed"}async function processStreamingResponse(e,t){const{onStart:o,onDelta:n,onChanges:i,onComplete:a,onError:r}=t;if(!e.body)throw new Error("No response body for streaming");const s=e.body.getReader(),l=new TextDecoder;let p="",c=null;try{for(;;){const{done:d,value:u}=await s.read();if(d)break;p+=l.decode(u,{stream:!0});const g=p.split(`
`);p=g.pop()||"";let m="",f="";for(const v of g)if(v.startsWith("event: "))m=v.slice(7);else if(v.startsWith("data: "))f=v.slice(6);else if(v===""&&m&&f){try{const h=JSON.parse(f);switch(m){case"start":o&&o(h);break;case"delta":n&&n(h);break;case"changes":i&&i(h);break;case"complete":c=h,a&&a(h);break;case"error":throw r&&r(h),new Error(h.error||"Stream error")}}catch(h){if(h.message!=="Stream error")console.warn("[Glyph] Failed to parse SSE data:",f);else throw h}m="",f=""}}}finally{s.releaseLock()}if(!c)throw new Error("Stream ended without complete event");return c}async function applyModifications(e){if(!e.trim()){showToast("Try a quick action above! They work instantly.","info",3e3),highlightQuickActions();return}if(userHasInteracted=!0,isProcessingModification){console.log("[Glyph] Modification already in progress, ignoring request");return}if(isInstantAction(e)){if(console.log("[Glyph] Applying instant action locally:",e),captureDiffBefore(),saveToUndoHistory(),simulateModification(e),markActionApplied(e),promptInput.value="",captureDiffAfter(),trackModifyRequest({sessionId,prompt:e}),hasShownFirstInstantWin?(showModificationSuccessToast(0,!0),updateResponseTimeBadge(0,!0),flashPreviewArea()):celebrateFirstInstantWin(),window.innerWidth<=480){const c=document.querySelector(".playground__preview-area");c&&setTimeout(()=>{c.scrollIntoView({behavior:"smooth",block:"start"})},300)}return}const t=currentHtml;captureDiffBefore(),saveToUndoHistory(),isProcessingModification=!0,showLoading(!0),setStatus("Applying..."),applyBtn.classList.add("loading"),applyBtn.disabled=!0,setSuggestionsDisabled(!0);const o=Date.now(),n=9e4,i=15e3,a=45e3;currentAbortController=new AbortController;const r=setTimeout(()=>{console.warn("[Glyph] Absolute timeout reached (90s)"),currentAbortController.abort()},n);let s=setTimeout(()=>{console.warn("[Glyph] No streaming activity for 15s, aborting"),currentAbortController.abort()},i);function l(){clearTimeout(s),s=setTimeout(()=>{console.warn("[Glyph] No streaming activity for 15s, aborting"),currentAbortController.abort()},i)}const p=setTimeout(()=>{isProcessingModification&&showToast("Almost there! Complex changes take a bit longer to perfect.","info",6e3)},a);try{async function c(m){try{const f=await m.json();return f.error||f.message||`Server error (${m.status})`}catch{return`Server error (${m.status})`}}if(sessionId){const m=await fetch(`${API_URL}/v1/modify?stream=true`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${DEMO_API_KEY}`},body:JSON.stringify({sessionId,prompt:e,html:currentHtml}),signal:currentAbortController.signal});if(window.__glyphRateLimit&&window.__glyphRateLimit.track(),!m.ok){m.status===429&&window.__glyphRateLimit&&window.__glyphRateLimit.onRateLimited();const y=await c(m);throw new Error(y)}let f=0,v="";const h=await processStreamingResponse(m,{onStart:y=>{console.log("[Glyph Stream] Started:",y.model),setStepperStep("analyze"),setStatus("AI is working...")},onDelta:y=>{if(f++,v+=y.html||"",l(),f===20&&setStepperStep("generate"),f%20===0){const w=Math.floor((Date.now()-o)/1e3);setStatus(`AI is working... ${w}s`)}},onChanges:y=>{console.log("[Glyph Stream] Changes:",y.changes),setStepperStep("generate")},onComplete:y=>{console.log("[Glyph Stream] Complete:",y.fastPath?"fast path":`${f} chunks`),setStepperStep("render"),elapsedTimerInterval&&(clearInterval(elapsedTimerInterval),elapsedTimerInterval=null)},onError:y=>{console.error("[Glyph Stream] Error:",y)}}),b=((Date.now()-o)/1e3).toFixed(1);if(isAiRefusalResponse(h.html)){console.warn("[Glyph] AI refusal detected, not rendering to document");const y=extractRefusalMessage(h.html);undoHistory.length>0&&(undoHistory.pop(),updateUndoButtonState()),showToast(y,"warning",5e3),setStatus("Request not supported"),setTimeout(()=>setStatus("Ready"),3e3),clearTimeout(r),clearTimeout(s);return}currentHtml=h.html,renderPreview(currentHtml,{changes:h.changes||[],previousHtml:t}),addHistoryEntry(e,currentHtml),previewContainer.classList.add("success"),setTimeout(()=>previewContainer.classList.remove("success"),500),flashPreviewArea(),consecutiveFailures=0,autoRetryCount=0,lastErrorWasSessionError=!1,showSuccessCta(),captureDiffAfter(),trackModifyRequest({sessionId,prompt:e}),showModificationSuccessToast(b),updateResponseTimeBadge(b),setStatus(`Done in ${b}s`),console.log(`[Glyph] Modification completed in ${b}s (${h.fastPath?"fast path":f+" stream chunks"})`),setTimeout(()=>setStatus("Ready"),2e3),sessionId&&startValidationPolling(sessionId),clearTimeout(r),clearTimeout(s);return}const d=await fetch(`${API_URL}/v1/modify`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${DEMO_API_KEY}`},body:JSON.stringify({html:currentHtml,instruction:e}),signal:currentAbortController.signal});if(window.__glyphRateLimit&&window.__glyphRateLimit.track(),!d.ok){d.status===429&&window.__glyphRateLimit&&window.__glyphRateLimit.onRateLimited();const m=await c(d);throw new Error(m)}const u=((Date.now()-o)/1e3).toFixed(1),g=await d.json();if(isAiRefusalResponse(g.html)){console.warn("[Glyph] AI refusal detected, not rendering to document");const m=extractRefusalMessage(g.html);undoHistory.length>0&&(undoHistory.pop(),updateUndoButtonState()),showToast(m,"warning",5e3),setStatus("Request not supported"),setTimeout(()=>setStatus("Ready"),3e3),clearTimeout(r),clearTimeout(s);return}currentHtml=g.html,renderPreview(currentHtml,{changes:g.changes||[],previousHtml:t}),addHistoryEntry(e,currentHtml),previewContainer.classList.add("success"),setTimeout(()=>previewContainer.classList.remove("success"),500),flashPreviewArea(),consecutiveFailures=0,autoRetryCount=0,lastErrorWasSessionError=!1,showSuccessCta(),captureDiffAfter(),trackModifyRequest({prompt:e}),showModificationSuccessToast(u),updateResponseTimeBadge(u),setStatus(`Done in ${u}s`),console.log(`[Glyph] Modification completed in ${u}s`),setTimeout(()=>setStatus("Ready"),2e3)}catch(c){clearTimeout(r),clearTimeout(s),console.error("[Glyph] API modification failed:",c),undoHistory.length>0&&(undoHistory.pop(),updateUndoButtonState());let d=c.message||"Unknown error";const u=Date.now()-o,g=c.name==="AbortError"&&u<n-1e3;if(c.name==="AbortError")if(g){showToast("Modification cancelled","info"),setStatus("Ready"),console.log("[Glyph] Modification cancelled by user");return}else d="timeout - request took too long (60s limit)";showApiError(d,e),setStatus("Error - See message"),setTimeout(()=>setStatus("Ready"),3e3)}finally{clearTimeout(r),clearTimeout(s),clearTimeout(p),currentAbortController=null,isProcessingModification=!1,showLoading(!1),applyBtn.classList.remove("loading"),applyBtn.disabled=!1,setSuggestionsDisabled(!1),promptInput.value=""}}let validationPollInterval=null,currentValidationToast=null;function startValidationPolling(e){validationPollInterval&&clearInterval(validationPollInterval);let t=0;const o=60;validationPollInterval=setInterval(async()=>{t++;try{const n=await fetch(`${API_URL}/v1/modify/validation-status?sessionId=${encodeURIComponent(e)}`,{headers:{Authorization:`Bearer ${DEMO_API_KEY}`}});if(n.ok){const i=await n.json();if(i.validationResult){clearInterval(validationPollInterval),validationPollInterval=null,i.validationResult.criticalCount>0&&!isInDemoMode?showValidationToast(i.validationResult,i.hasAutoFix,e):i.validationResult.criticalCount>0&&isInDemoMode?(console.log("[Glyph] Demo mode: Suppressing validation toast ("+i.validationResult.criticalCount+" critical issues)"),showValidationSuccessToast()):i.validationResult.warningCount>0?(console.log("[Glyph] Self-check: "+i.validationResult.warningCount+" minor warnings (not shown to user)"),showValidationSuccessToast()):(console.log("[Glyph] Self-check passed - no issues detected"),showValidationSuccessToast());return}}}catch(n){console.warn("[Glyph] Validation poll error:",n)}t>=o&&(clearInterval(validationPollInterval),validationPollInterval=null,console.log("[Glyph] Validation poll timeout - validation still pending"),showValidationTimeoutMessage())},1e3)}window.addEventListener("pagehide",()=>{validationPollInterval&&(clearInterval(validationPollInterval),validationPollInterval=null)});function showValidationTimeoutMessage(){if(!userHasInteracted)return;const e=document.createElement("div");e.className="validation-toast validation-toast--info",e.innerHTML=`
      ...
      `,document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("visible")),setTimeout(()=>{e.classList.remove("visible"),setTimeout(()=>e.remove(),300)},5e3)}function showValidationToast(e,t,o){currentValidationToast&&currentValidationToast.remove();const n=document.createElement("div");n.className="validation-toast";const i=e.criticalCount>0,a="validation-toast__icon--warning",r=i?`<svg class="validation-toast__icon ${a}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M8.485 2.495c.673-1.167 2.357-1.167 3.03 0l6.28 10.875c.673 1.167-.17 2.625-1.516 2.625H3.72c-1.347 0-2.189-1.458-1.515-2.625L8.485 2.495zM10 5a.75.75 0 01.75.75v3.5a.75.75 0 01-1.5 0v-3.5A.75.75 0 0110 5zm0 9a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd"/></svg>`:`<svg class="validation-toast__icon ${a}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M8.485 2.495c.673-1.167 2.357-1.167 3.03 0l6.28 10.875c.673 1.167-.17 2.625-1.516 2.625H3.72c-1.347 0-2.189-1.458-1.515-2.625L8.485 2.495zM10 5a.75.75 0 01.75.75v3.5a.75.75 0 01-1.5 0v-3.5A.75.75 0 0110 5zm0 9a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd"/></svg>`,s=(e.issues||[]).slice(0,3).map(l=>`<div class="validation-toast__issue validation-toast__issue--${l.severity==="critical"?"critical":"warning"}">${l.description}</div>`).join("");n.innerHTML=`
        ...
        `}
      `,n.querySelector(".validation-toast__close").addEventListener("click",()=>{dismissValidationToast(n)}),n.querySelectorAll("[data-action]").forEach(l=>{l.addEventListener("click",p=>{p.target.dataset.action==="apply-fix"?applyValidationFix(o,n):dismissValidationToast(n)})}),document.body.appendChild(n),currentValidationToast=n,requestAnimationFrame(()=>{n.classList.add("visible")}),setTimeout(()=>{document.body.contains(n)&&dismissValidationToast(n)},15e3)}function dismissValidationToast(e){e.classList.remove("visible"),setTimeout(()=>{e.remove(),currentValidationToast===e&&(currentValidationToast=null)},300)}function showValidationSuccessToast(){currentValidationToast&&currentValidationToast.remove();const e=document.createElement("div");e.className="validation-toast validation-toast--success";const t=`<svg class="validation-toast__icon validation-toast__icon--success" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
      ...
      `,document.body.appendChild(e),currentValidationToast=e,requestAnimationFrame(()=>{e.classList.add("visible")}),setTimeout(()=>{document.body.contains(e)&&dismissValidationToast(e)},3e3)}async function applyValidationFix(e,t){const o=t.querySelector('[data-action="apply-fix"]');o.textContent="Applying...",o.disabled=!0;try{const n=await fetch(`${API_URL}/v1/modify/apply-fix`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${DEMO_API_KEY}`},body:JSON.stringify({sessionId:e})});n.ok?(currentHtml=(await n.json()).html,renderPreview(currentHtml),o.textContent="Fixed!",o.style.background="var(--color-success)",previewContainer.classList.add("success"),setTimeout(()=>previewContainer.classList.remove("success"),500),flashPreviewArea(),setTimeout(()=>{dismissValidationToast(t)},1e3)):(o.textContent="Failed",o.style.background="var(--color-error)",setTimeout(()=>{o.textContent="Apply Fix",o.style.background="",o.disabled=!1},2e3))}catch(n){console.error("[Glyph] Apply fix error:",n),o.textContent="Error",o.disabled=!1}}function injectWatermark(e,t="DRAFT"){if(e.includes("glyph-watermark"))return e.replace(/(<div class="glyph-watermark"[^>]*>)[^<]*(<\/div>)/i,`$1${t}$2`);const n=`<div class="glyph-watermark" style="${`
        position: fixed;
        top: 50%;
        left: 50%;
        ...
        letter-spacing: 20px;
        user-select: none;
      `.replace(/\n\s*/g," ")}">${t}</div>`;return e.replace("</body>",`${n}</body>`)}function injectQrCode(e){if(e.includes("glyph-qr-code"))return e;const n=`<div class="glyph-qr-code" style="${`
        position: fixed;
        top: 20px;
        right: 20px;
        width: 80px;
      ...
      </head>`)}function injectDate(e){if(e.includes("glyph-date-badge"))return e;const o=new Date().toLocaleDateString("en-US",{year:"numeric",month:"long",day:"numeric"}),i=`<div class="glyph-date-badge" style="${`
        position: fixed;
        top: 20px;
        right: 110px;
        background: #f8fafc;
      ...
      `.replace(/\n\s*/g," ")}">${o}</div>`;return e.replace("</body>",`${i}</body>`)}function injectBorder(e){return e.includes("glyph-document-border")?e:e.replace("</head>",`
      ...
      </head>`)}function injectThankYouNote(e){if(e.includes("glyph-thank-you-note"))return e;const o=`<div class="glyph-thank-you-note" style="${`
        margin: 24px 20px;
        padding: 20px;
      ...
      </div>`;return e.replace("</body>",`${o}</body>`)}function injectSignature(e){if(e.includes("glyph-signature-line"))return e;const t=new Date().toLocaleDateString("en-US",{year:"numeric",month:"long",day:"numeric"}),n=`<div class="glyph-signature-line" style="${`
        margin: 32px 20px;
        padding: 24px;
        font-family: sans-serif;
      ...
      `.replace(/\n\s*/g," ")}">
        ...
        <div style="font-size: 12px; color: #1a1a1a; line-height: 1.5;">${t}</div>
          ...
          </tr>`}return e.replace(/<tbody>[\s\S]*?<\/tbody>/,`<tbody>${n}</tbody>`)}function simulateModification(e){const t=e.toLowerCase();let o=currentHtml;if(t.includes("terms")||t.includes("payment terms")||t.includes("net ")){const n=extractTermsText(e);o=injectPaymentTerms(o,n)}if(t.includes("watermark")){let n="DRAFT";t.includes("paid")?n="PAID":t.includes("confidential")?n="CONFIDENTIAL":t.includes("approved")?n="APPROVED":t.includes("void")?n="VOID":t.includes("sample")&&(n="SAMPLE"),o=injectWatermark(o,n)}t.includes("qr")&&(o=injectQrCode(o)),t.includes("group")&&(t.includes("category")||t.includes("subtotal"))&&(o=injectGroupByCategory(o)),t.includes("bold")&&(t.includes("header")||t.includes("title"))&&(o=injectBoldHeader(o)),t.includes("date")&&(t.includes("add")||t.includes("today"))&&(o=injectDate(o)),t.includes("border")&&(o=injectBorder(o)),t.includes("thank")&&(t.includes("you")||t.includes("note"))&&(o=injectThankYouNote(o)),t.includes("signature")&&(o=injectSignature(o)),t.includes("blue")?o=changeAccentColor(o,"#3B82F6"):t.includes("purple")||t.includes("stripe")?o=changeAccentColor(o,"#635BFF"):t.includes("red")?o=changeAccentColor(o,"#EF4444"):t.includes("green")?o=changeAccentColor(o,"#22C55E"):t.includes("orange")?o=changeAccentColor(o,"#F97316"):t.includes("pink")?o=changeAccentColor(o,"#EC4899"):(t.includes("gold")||t.includes("yellow"))&&(o=changeAccentColor(o,"#EAB308")),currentHtml=o,renderPreview(currentHtml),addHistoryEntry(e,currentHtml),previewContainer.classList.add("success"),setTimeout(()=>previewContainer.classList.remove("success"),500),flashPreviewArea(),setStatus("Demo: Updated!"),setTimeout(()=>setStatus("Demo Mode"),2e3)}function generateEnhancedHtml(e,t,o){const n=o.style==="stripe",i=o.style==="apple",a=o.watermark?`
        <div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%) rotate(-45deg); font-size: 80px; font-weight: 900; color: rgba(0,0,0,0.06); pointer-events: none; white-space: nowrap; z-index: 1;">${o.watermark}</div>
      `:"",r=o.qrCode?`
      ...
      `:"",s=o.terms?`
        <div style="margin-top: 24px; padding: 16px; background: #f9fafb; border-radius: 8px; border-left: 4px solid ${t};">
          <div style="font-size: 10px; text-transform: uppercase; letter-spacing: 0.1em; color: #666; margin-bottom: 8px; font-weight: 600;">Payment Terms</div>
          <div style="font-size: 12px; color: #1a1a1a;">${o.terms}</div>
        </div>
      `:"",l=o.signature?`
      ...
      `:"",p=o.progressBar?`
        <div style="margin-top: 20px; padding: 16px; background: linear-gradient(135deg, ${t}15, ${t}05); border-radius: 8px;">
          <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
            ...
            <span style="font-size: 11px; font-weight: 600; color: ${t};">${o.progressBar}% Paid</span>
          </div>
          <div style="height: 8px; background: #e5e5e5; border-radius: 4px; overflow: hidden;">
            <div style="height: 100%; width: ${o.progressBar}%; background: linear-gradient(90deg, ${t}, ${t}cc); border-radius: 4px;"></div>
          </div>
          <div style="font-size: 10px; color: #666; margin-top: 6px;">$${(e.totals.total*o.progressBar/100).toFixed(2)} of $${e.totals.total.toFixed(2)} received</div>
        </div>
      `:"";return`<!DOCTYPE html>
<html>
<head>
...
<body>
  ...
  <div class="header">
    ...
    <div class="title">${n?"Invoice":"Quote"}</div>
  </div>
    ...
    <tbody>
      ...
      <tr>
        <td>
          ...
          <div class="item-details">${u.details}</div>
        </td>
        <td>${u.quantity}</td>
        <td>$${u.unitPrice}</td>
        <td>$${u.total}</td>
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .notes { background: ${o}; padding: 16px; border-radius: 8px; margin-top: 32px; }
  ...
  <table class="line-items">
    ...
    <tbody>
      <tr>
        ...
        <td>$${(parseFloat(u)*parseFloat(g)||500).toFixed(2)}</td>
      </tr>
      <tr>
...
</html>`}function generateReceiptTemplate(e,t,o,n,i,a,r){const s=findFieldValue(n,["name","customer","client","contact"],"Customer Name"),l=findFieldValue(n,["email","mail"],"customer@example.com"),p=findFieldValue(n,["item","product","service","description"],"Product/Service"),c=findFieldValue(n,["payment","method","card"],"Credit Card");return`<!DOCTYPE html>
<html>
<head>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .receipt-meta { background: ${o}; padding: 16px; border-radius: 8px; margin-bottom: 24px; }
...
</html>`}function generatePackingSlipTemplate(e,t,o,n,i,a,r){const s=findFieldValue(n,["name","customer","client","contact","recipient"],"Recipient Name"),l=findFieldValue(n,["address","location","street","shipping"],"456 Delivery Lane"),p=findFieldValue(n,["city","town"],"Anytown"),c=findFieldValue(n,["item","product","sku","description"],"Product Item"),d=findFieldValue(n,["qty","quantity","units","count"],"2");return`<!DOCTYPE html>
<html>
<head>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .address-content { line-height: 1.8; padding: 12px; background: ${o}; border-radius: 8px; }
  .address-name { font-weight: 700; font-size: 14px; }
  .order-meta { background: ${o}; padding: 16px 24px; border-radius: 8px; margin-bottom: 32px; display: flex; gap: 40px; }
  .meta-block { }
  .meta-label { font-size: 10px; text-transform: uppercase; letter-spacing: 0.1em; color: ${t}; margin-bottom: 4px; }
  .meta-value { font-weight: 600; font-size: 14px; }
...
</html>`}function generateReportTemplate(e,t,o,n,i,a,r){const s=findFieldValue(n,["title","name","report","subject"],"Data Report");return`<!DOCTYPE html>
<html>
<head>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .meta-bar { display: flex; gap: 32px; background: ${o}; padding: 16px 24px; border-radius: 8px; margin-bottom: 32px; }
  .meta-item { }
  .meta-label { font-size: 10px; text-transform: uppercase; letter-spacing: 0.1em; color: ${t}; margin-bottom: 4px; }
  .meta-value { font-weight: 600; }
  ...
  .summary-card { flex: 1; background: ${o}; padding: 20px; border-radius: 8px; text-align: center; }
  ...
  .data-table tr:nth-child(even) { background: ${o}; }
  .footer { margin-top: 48px; padding-top: 16px; border-top: 1px solid #e5e5e5; text-align: center; color: #999; font-size: 10px; }
...
<body>
  ...
  <div class="section">
    ...
    <div class="summary-cards">
      ...
      <div class="summary-card">
        <div class="card-value">${Object.keys(n).length}</div>
...
</html>`}function generateDataTableTemplate(e,t,o,n,i,a,r){return`<!DOCTYPE html>
<html>
<head>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .doc-title { font-size: 28px; font-weight: 700; color: ${e}; text-transform: uppercase; letter-spacing: 0.05em; }
  .meta { display: flex; gap: 40px; background: ${o}; padding: 16px 24px; border-radius: 8px; margin-bottom: 24px; }
  .meta-item { }
  .meta-label { font-size: 10px; text-transform: uppercase; letter-spacing: 0.1em; color: ${t}; margin-bottom: 4px; }
  .meta-value { font-weight: 600; font-size: 14px; }
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td><div class="field-value">${n[s]||"{{"+s+"}}"}</div></td>
        </tr>
        `).join("")}
      </tbody>
    </table>
