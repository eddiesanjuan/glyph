    ...
    paths: {
      ...
      "/v1/create": {
        post: {
          ...
          tags: ["Core"],
          security: [{ bearerAuth: [] }],
          requestBody: { required: true, content: { "application/json": { schema: { type: "object", properties: { templateId: { type: "string", description: "Built-in template ID (optional, auto-detected if omitted)" }, data: { type: "object", description: "Document data (field values)" }, intent: { type: "string", description: "Natural language description of desired output" }, style: { type: "string", enum: ["modern", "professional", "classic", "vibrant", "minimal", "invoice", "report"] }, format: { type: "string", enum: ["pdf", "png"], default: "pdf" } }, required: ["data"] } } } },
          responses: { "200": { description: "Generated PDF with metadata", content: { "application/json": { schema: { type: "object", properties: { success: { type: "boolean" }, format: { type: "string", enum: ["pdf", "png"] }, url: { type: "string", description: "Base64 data URL of the generated file" }, size: { type: "integer", description: "File size in bytes" }, filename: { type: "string" }, expiresAt: { type: "string", format: "date-time" }, analysis: { type: "object", properties: { detectedType: { type: "string" }, confidence: { type: "number" }, fieldsIdentified: { type: "array", items: { type: "string" } }, layoutDecisions: { type: "array", items: { type: "string" } } } }, sessionId: { type: "string", description: "Session ID for subsequent modify calls" }, _links: { type: "object", properties: { modify: { type: "string" }, generate: { type: "string" } } } }, required: ["success", "format", "url", "size", "sessionId"] } } } } },
        },
      },
      "/v1/airtable/connect": {
        post: {
